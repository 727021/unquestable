...
try {
  ...
  for (const e of data.expansions) {
    ...
    if (existing) {
      console.info(
        `Expansion ${existing.name} (${existing.type}) already exists (id: ${existing.id})`
      )
      process.exit(0)
    }
    try {
      ...
      for (const c of e.campaigns) {
        campaign = await prisma.campaign.create({
          data: {
            ...
            missionSlots: {
              create: c.missions.map((m, i) => ({
                index: i,
                threat: m.threat,
                type: m.type,
                itemTiers: m.itemTiers
              }))
            },
            expansionId: expansion.id
          }
      ...
      for (const m of e.missions) {
        ...
        for (const status of ['win', 'loss', 'all']) {
          ...
          if (placeholders) {
            // insert placeholders
            for (const [k, { type, label, ...validation }] of Object.entries(
              placeholders
            )) {
      ...
      console.log(
        `Seeded data for expansion ${expansion.name} (${expansion.type}) [id: ${expansion.id}]`
      )
    } catch (error) {
